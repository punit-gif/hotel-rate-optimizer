...
from backend.db import engine                     # <-- use shared, normalized engine (psycopg v3)

load_dotenv()

# Kept for compatibility, but not used now that we import the shared engine.
POSTGRES_URL = os.getenv("POSTGRES_URL")


def read_tables(engine_):
    res = pd.read_sql(
        "SELECT stay_date, room_type, occupancy, adr FROM reservations ORDER BY stay_date, room_type",
        engine_
    )
    try:
        comp = pd.read_sql(
            "SELECT stay_date, competitor, room_type, rate FROM competitor_rates",
            engine_
        )
    except Exception:
...
def forecast_demand(train_df, feat_cols):
    ...
    try:
        ...
        # Fallback: 7-day rolling mean per room_type
        return train_df.groupby('room_type')['target'].transform(
            lambda s: s.rolling(7, min_periods=2).mean().fillna(method='bfill')
        ).clip(0, 1)


def main():
    # Use shared engine from backend.db; no local create_engine call needed
    ...
    for d in horizon:
        for rt in room_types:
            ...
            if last.empty:
                ...
                comp_median = (
                    comp[(comp['stay_date'] == d.date()) & (comp['room_type'] == rt)]['rate'].median()
    ...
    with engine.begin() as conn:
        for _, r in fut.iterrows():
            conn.execute(
                text("DELETE FROM forecasts WHERE stay_date=:stay AND room_type=:room;"),
